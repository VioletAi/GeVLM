...
from models.positional_projector import PositionalEmbeddingProjector
...
class Chat3D(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_attention = nn.MultiheadAttention(embed_dim=self.scene_dim, num_heads=4)
        self.pos_embed_projector = PositionalEmbeddingProjector(input_dim=self.scene_dim, output_dim=self.scene_dim)
    ...
    def forward_train(self, scene_feat, scene_img_feat, scene_locs, scene_mask, obj_ids, assigned_ids, questions, answers, scene_id, isDetailedScanrefer, is_eval=False, **kwargs):
        ...
        pos_embed = self.pos_embed_projector(pos_embed)
        
        # transpose for multihead attention: from [batch_size, num_objects, features]
        # to [num_objects, batch_size, features] as multihead attention expects inputs in [seq_len, batch, features]
        pos_embed = pos_embed.transpose(0, 1)
        attn_output, _ = self.pos_attention(pos_embed, pos_embed, pos_embed)
        # transpose back to [batch_size, num_objects, features]
        attn_output = attn_output.transpose(0, 1)

        # Concatenate the attended positional embeddings with the object embeddings
        object_embed = torch.cat((object_embed, attn_output), dim=2)
        
        # pos_embed = self.pos_proj(pos_embed)
        # object_embed = torch.cat((object_embed, pos_embed), dim=2)
        #end add positional information
